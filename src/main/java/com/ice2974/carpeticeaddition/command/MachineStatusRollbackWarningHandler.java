...
public final class MachineStatusRollbackWarningHandler {
    ...
    public static void warnIfNeeded(MinecraftServer server, ServerPlayer player, String rawInput) {
        ...
        player.displayClientMessage(titleLine(), false);
        player.displayClientMessage(commandLine(rawInput), false);
        player.displayClientMessage(introLine(), false);
        for (MachineRecord record : runningMachines) {
            player.displayClientMessage(formatMachineLine(record, MachineStatusKind.RUNNING), false);
        }
        for (MachineRecord record : invalidMachines) {
            player.displayClientMessage(formatMachineLine(record, MachineStatusKind.INVALID), false);
        }
        player.displayClientMessage(footerLine(), false);
        LOGGER.debug(
