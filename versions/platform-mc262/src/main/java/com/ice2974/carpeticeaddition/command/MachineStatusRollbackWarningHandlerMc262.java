...
public final class MachineStatusRollbackWarningHandlerMc262 {
    private static final Logger LOGGER = LoggerFactory.getLogger(MachineStatusRollbackWarningHandlerMc262.class);

    private MachineStatusRollbackWarningHandlerMc262() {
    }
    ...
    public static void warnIfNeeded(MinecraftServer server, ServerPlayer player, String rawInput) {
        ...
        List<MachineRecord> runningMachines = MachineStatusCommandMc262.getMachineRecordsByStatus(server, MachineStatusKind.RUNNING);
        List<MachineRecord> invalidMachines = MachineStatusCommandMc262.getMachineRecordsByStatus(server, MachineStatusKind.INVALID);
