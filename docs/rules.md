...
### 基岩版三叉戟移植 (bedrockTridentPort)

使落地后的三叉戟在重新移动时仍能像基岩版一样对实体造成伤害。落地静止的三叉戟不会因实体靠近或重叠而持续受到伤害；每次落地期至多授予一次重新命中资格，命中后资格即被消耗，需再次落地并被重新推动才能再次造成伤害。投掷、飞行、首次命中、忠诚返回、附魔与伤害来源均保持 Java 原版行为，规则关闭后立即恢复原版。
