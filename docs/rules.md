...
数值单位为游戏刻；只允许 `1` 至 `72000` 的整数或 `freeze`，其中 `72000` 是规则基础延迟的最大输入值。`freeze` 的含义为冻结水的计划流体刻（不流动），但不影响水桶放置、下界水蒸发、炼药锅、滴水石填充、实体与流体交互、流体动画等非计划流体刻行为。数字值发生变化时，只影响之后新创建或再次调度的流体计划刻，不遍历世界、不强制重排已经存在的计划刻。部分原版世界生成和结构生成代码会为流体首次激活显式创建延迟为 0 的内部计划刻；该首次激活的创建延迟不读取数值规则，但到期执行时仍受 `freeze` 拦截，之后由流体逻辑创建或再次调度的计划刻才使用规则基础延迟。数值设置过低可能对服务器性能造成影响。
`freeze` 通过周期性保活计划刻暂停流体流动，并非像原版 `/tick freeze` 一样精确保留计划刻的剩余时间。只有某位置的原计划刻已经在冻结期间到期并进入保活循环后，解除冻结时才会等待下一次保活计划刻；在区块持续加载且可执行计划刻、调度器没有积压时，名义上最多额外等待约 5 游戏刻（水）、30 游戏刻（普通维度岩浆）或 10 游戏刻（ultrawarm / FAST_LAVA 维度岩浆）。尚未进入保活循环的远期计划刻仍保留原到期时间。保活是新创建的普通优先级计划刻，不保留旧计划刻的精确优先级和同刻顺序。大量位置进入保活循环时可能产生周期性轮询和同步恢复峰值。
...
数值单位为游戏刻；只允许 `1` 至 `72000` 的整数或 `freeze`，其中 `72000` 是规则基础延迟的最大输入值，不是岩浆最终实际计划间隔的绝对上限。在 ultrawarm / FAST_LAVA 维度（如原版下界）中，岩浆先使用 `max(1, 基础延迟 / 3)`；在原版特定液面上升状态下，之后仍有 75% 概率将该结果乘以 4。因此基础延迟为 `72000` 时，普通维度和 FAST_LAVA 维度中的条件性四倍间隔可分别达到 `288000` 和 `96000` 游戏刻。`freeze` 的含义为冻结岩浆的流动计划流体刻（不流动），但不影响岩浆随机刻（随机点火）。数字值发生变化时，只影响之后新创建或再次调度的流体计划刻，不遍历世界、不强制重排已经存在的计划刻。部分原版世界生成和结构生成代码会为流体首次激活显式创建延迟为 0 的内部计划刻；该首次激活的创建延迟不读取数值规则，但到期执行时仍受 `freeze` 拦截，之后由流体逻辑创建或再次调度的计划刻才使用规则基础延迟。数值设置过低可能对服务器性能造成影响。
`freeze` 通过周期性保活计划刻暂停流体流动，并非像原版 `/tick freeze` 一样精确保留计划刻的剩余时间。只有某位置的原计划刻已经在冻结期间到期并进入保活循环后，解除冻结时才会等待下一次保活计划刻；在区块持续加载且可执行计划刻、调度器没有积压时，名义上最多额外等待约 5 游戏刻（水）、30 游戏刻（普通维度岩浆）或 10 游戏刻（ultrawarm / FAST_LAVA 维度岩浆）。尚未进入保活循环的远期计划刻仍保留原到期时间。保活是新创建的普通优先级计划刻，不保留旧计划刻的精确优先级和同刻顺序。大量位置进入保活循环时可能产生周期性轮询和同步恢复峰值。
