...
重新武装资格不跨区块卸载或服务器重启保留，需重新经历插地并再次移动才会恢复伤害能力。
...
自定义水的流动延迟，单位为游戏刻：整数（1 至 72000）强制覆盖；freeze 冻结流动调度；vanilla 不接管，交由原版及其它模组计算。
...
自定义岩浆的流动延迟，单位为游戏刻：整数（1 至 72000）强制覆盖；freeze 冻结流动调度；vanilla 不接管，交由原版及其它模组计算。\
ultrawarm 维度（如下界）中的延迟为设置值的三分之一且最低为 1。
