...
当村民被命名为 iron_golem 时，优化其 AI：跳过工作、闲逛张望与展示交易物品等无关行为，并抑制寻找工作站与主动寻路捡取物品，仅保留生成铁傀儡所必需的相关行为。

优化期间村民仍会照常认床并按日程睡觉、保持恐慌逃离与僵尸惊吓反应、保留村民间闲聊聚会等铁傀儡生成触发链路，不改变原版生成条件、概率、冷却与生成位置判定；被跳过的只有工作、游玩、闲逛张望、繁殖、展示交易物品、寻找与认领工作站、主动寻路捡取物品等无关行为。命名期间村民不会寻找或获得职业，也不会主动寻路捡取物品（物品进入原版近身拾取范围时仍可被原版拾取路径捡起）。名称匹配比较纯文本内容、区分大小写，需精确等于小写 `iron_golem`，颜色、粗体等样式不影响匹配。改名、移除名称或关闭规则后，相关任务自下一次启动判定起恢复完整原版行为（进行中的无关行为会自然结束当前周期），已有原版记忆保留并继续使用，缺失记忆按原版自身周期自然重建，不写入任何依赖本模组的存档数据。
