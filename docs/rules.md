...
修复方式为 backport 原版 1.21.2 对合成类输出槽 Ctrl+Q 行为的修复（该改动同期修复了 MC-135971）：在逻辑服务端连续取出并丢弃相同产物，直到输出槽为空或产物发生变化，因此累计产出可能超过一组（例如 64 个原料配 1→2 产量的配方会丢出 128 个产物）。普通 Q、Shift 点击、鼠标左右键等取出方式不受影响；该修复纯服务端生效，客户端无需安装本模组，仅客户端安装本模组时也不会改变原版行为。
