...
跳过原版的文本字符合法性检查，允许使用原版通常会拦截的字符，例如分节符。粘贴文本仍会移除 CR，但不再因原版格式清理而删除分节符序列；1.21.1–1.21.5 的旧版书名界面额外支持 Ctrl+V。

该规则影响调用原版公共文本字符检查的界面和处理路径，包括聊天、命令、告示牌、书本和铁砧。界面专属的长度、格式、语法和业务谓词，以及服务端文本过滤、数据包限制和聊天签名验证仍然生效；不调用该公共方法的第三方自定义文本框不在保证范围内。

仅客户端安装并连接纯原版服务器时，可发送 `/carpet disableIllegalTextCharacterCheck true` 在当前客户端进程内临时切换规则。该命令仍会发送给服务器，因此原版服务器通常会返回未知命令，服务端也仍可能过滤或拒绝相关文本。连接安装 Carpet 但未安装本扩展的服务器时，不保证存在可靠的本地开关。

告示牌格式代码能否从编辑阶段保留到存档，取决于安装方式和服务器同步的规则值：

| 安装方式 | 告示牌行为 |
| --- | --- |
| 单人游戏 | 客户端与集成服务端共享当前规则值；规则为 `true` 时可放宽输入并保留服务端实际收到的原始文本。 |
| 仅客户端安装 | 客户端规则为 `true` 时可放宽本地输入；未安装本 Mod 的服务器仍会按原版清理告示牌格式代码。 |
| 仅服务端安装 | 服务端规则为 `true` 时可保留客户端实际提交的、包含分节符的告示牌原始文本；原版客户端通常无法通过普通文本输入界面输入这些字符。 |
| 客户端和服务端均安装 | 服务端同步的规则值会更新客户端同名规则；同步后的规则为 `true` 时，可保留从客户端编辑、数据包提交到服务端保存的完整链路。 |

服务端保存的是 `SignText` 中包含分节符的原始 literal 字符串，客户端在渲染时才解释受支持的格式序列；这不表示服务端生成了结构化颜色 `Style`。文本过滤、数据包限制、界面长度和行宽限制、封蜡、权限与编辑者检查、正反面判断，以及书本和聊天的其他原版校验仍然生效。
