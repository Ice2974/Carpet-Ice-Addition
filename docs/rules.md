...
自定义水的流动延迟，单位为游戏刻；默认 `vanilla` 表示不干预原版流动延迟。设置为 `freeze` 时冻结水的流动相关计划流体刻。设为正整数（包括 5）时强制使用该延迟；仅 `vanilla` 状态下其他修改流体刻延迟的模组才会生效。
...
自定义岩浆的流动延迟，单位为游戏刻；默认 `vanilla` 表示不干预原版流动延迟。ultrawarm 维度中的延迟为设置值的三分之一且最低为 1；设置为 `freeze` 时冻结岩浆流动，但不影响随机刻。设为正整数（包括 30）时强制使用该延迟；仅 `vanilla` 状态下其他修改流体刻延迟的模组才会生效。
