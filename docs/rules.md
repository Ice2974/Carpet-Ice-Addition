...
该规则影响所有调用原版公共聊天字符合法性检查方法的原版界面和处理路径，包括聊天、命令、告示牌、书本和铁砧等复用原版文本输入组件的场景。界面专属的长度、格式、语法和业务谓词，以及服务端文本过滤、数据包限制和聊天签名验证仍然生效；完全使用自定义文本框且不调用该公共方法的第三方 Mod 不在保证范围内。

仅客户端安装并连接纯原版服务器时，可发送 `/carpet disableIllegalChatCharacterCheck true` 在当前客户端进程内临时切换规则。该命令仍会发送给服务器，因此原版服务器通常会返回未知命令；服务端也仍可能过滤、拒绝相关文本或断开连接。连接安装 Carpet 但未安装本扩展的服务器时，不保证存在可靠的本地开关。

告示牌格式代码能否从编辑阶段保留到存档，还取决于安装方式和服务器同步的规则值：

| 安装方式 | 告示牌行为 |
| --- | --- |
| 仅客户端安装 | 客户端进程内的规则为 `true` 时可放宽本地键入和粘贴；未安装本 Mod 的服务器仍会在保存告示牌时按原版删除格式代码。 |
| 仅服务端安装 | 服务器规则为 `true` 时可保留客户端实际提交的、包含分节符的告示牌原始文本；原版客户端通常无法通过普通文本输入界面输入这些字符。 |
| 客户端和服务端均安装 | 服务器同步的规则值会覆盖客户端进程内同名规则的临时值；同步后的规则为 `true` 时，可保留从客户端编辑、数据包提交到服务端保存的完整链路。 |

服务端保存的是 `SignText` 中包含分节符的原始 literal 字符串，客户端在渲染时才将受支持的序列解释为格式代码；这不表示服务端生成了结构化颜色 `Style`。服务端文本过滤、数据包限制、界面长度和行宽限制、封蜡与编辑权限、正反面判断，以及书本和聊天的其他原版校验仍然生效；本规则没有为书本新增服务端格式清理绕过。
