...
Trims the AI of villagers named trade to the minimum required by a fixed trading hall: only vanilla restocking, schedule switching, swimming, and job site validation are kept, while trading, leveling up, and new offers are unaffected; set up the profession and job site first and keep the villager right beside its workstation.

This rule targets finished fixed trading halls: the villager should already have its final profession and a bound job site before being named, and should be kept within roughly 1.73 blocks of the workstation (placing it directly beside or adjacent to the workstation is recommended). While optimized, the villager no longer walks or pathfinds on its own, no longer seeks, competes for, or switches workstations, no longer opens doors, faces players, wanders, socializes, panics, sleeps, breeds, spreads gossip, takes part in iron golem summoning, or picks up items, and no longer walks to its workstation on its own; restocking keeps the vanilla work behavior with all vanilla limits preserved, including the daily restock count, the interval between restocks, and the cross-day reset, and existing gossip and reputation keep affecting trade prices. If the workstation is removed or invalidated, the villager stops restocking and keeps its profession, but does not re-claim a workstation on its own; before changing the profession, swapping the workstation, or moving the villager back into place, rename it or turn the rule off first so the vanilla AI recovers. The villager can still be moved by external force, boats, minecarts, or flowing water (it never pathfinds onto vehicles on its own) and still tries to jump out of water. Name matching is case sensitive and must equal the lowercase name `trade` exactly. After renaming, removing the name, or turning the rule off, the villager recovers its full vanilla AI through the vanilla brain rebuild path, and no mod-dependent data is written to the save.
