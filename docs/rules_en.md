...
The unit is game ticks; only integers from `1` through `72000` or `freeze` are accepted, and `72000` is the maximum input for the rule's base delay. `freeze` freezes water's scheduled fluid ticks (no flow), but does not affect water bucket placement, water evaporation in the Nether, cauldrons, dripstone filling, entity-fluid interactions, or fluid animations. When the numeric value changes, it only affects newly created or re-scheduled fluid ticks; it does not scan the world or force-reschedule existing ticks. Some vanilla world-generation and structure-generation paths explicitly create an internal zero-delay tick for a fluid's initial activation; creating that tick does not read the numeric rule, but its execution is still intercepted by `freeze`, and ticks subsequently created or re-scheduled by fluid logic use the configured base delay. Setting the value too low may impact server performance.
`freeze` suspends fluid flow using periodic keep-alive scheduled ticks rather than the precise remaining-time pause of vanilla `/tick freeze`. Only after the position's original tick has expired while frozen and entered the keep-alive loop does unfreezing wait for the next keep-alive tick; while the chunk remains loaded and tickable and the scheduler is not backlogged, the nominal extra wait is at most about 5 game ticks for water, 30 for non-ultrawarm lava, or 10 for ultrawarm / FAST_LAVA lava. A future tick that has not entered that loop retains its original trigger time. A keep-alive is a newly created normal-priority tick and does not preserve the original tick's exact priority or same-tick order. Large numbers of positions in keep-alive loops may cause periodic polling and synchronized recovery spikes.
...
The unit is game ticks; only integers from `1` through `72000` or `freeze` are accepted, and `72000` is the maximum input for the rule's base delay rather than an absolute maximum for lava's final scheduled interval. In ultrawarm / FAST_LAVA dimensions (such as the vanilla Nether), lava first uses `max(1, base delay / 3)`; for a specific vanilla rising-level state, there is then still a 75% chance that the result is multiplied by 4. A base delay of `72000` can therefore produce conditional four-times intervals of `288000` game ticks in normal dimensions and `96000` in FAST_LAVA dimensions. `freeze` freezes lava's scheduled fluid ticks (no flow), but does not affect lava random ticks or random fire ignition. When the numeric value changes, it only affects newly created or re-scheduled fluid ticks; it does not scan the world or force-reschedule existing ticks. Some vanilla world-generation and structure-generation paths explicitly create an internal zero-delay tick for a fluid's initial activation; creating that tick does not read the numeric rule, but its execution is still intercepted by `freeze`, and ticks subsequently created or re-scheduled by fluid logic use the configured base delay. Setting the value too low may impact server performance.
`freeze` suspends fluid flow using periodic keep-alive scheduled ticks rather than the precise remaining-time pause of vanilla `/tick freeze`. Only after the position's original tick has expired while frozen and entered the keep-alive loop does unfreezing wait for the next keep-alive tick; while the chunk remains loaded and tickable and the scheduler is not backlogged, the nominal extra wait is at most about 5 game ticks for water, 30 for non-ultrawarm lava, or 10 for ultrawarm / FAST_LAVA lava. A future tick that has not entered that loop retains its original trigger time. A keep-alive is a newly created normal-priority tick and does not preserve the original tick's exact priority or same-tick order. Large numbers of positions in keep-alive loops may cause periodic polling and synchronized recovery spikes.
