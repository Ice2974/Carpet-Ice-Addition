...
Trims the AI of villagers named trade to the minimum required by a fixed trading hall: the villager never moves or pathfinds on its own but can claim a workstation within roughly 1.73 blocks and loses or regains its profession by vanilla rules, with all vanilla restock limits kept; trading, leveling up, and new offers are unaffected.

This rule targets fixed trading halls: the workstation must be placed within roughly 1.73 blocks of the villager (the restocking distance); directly beside or adjacent placement is recommended. Claiming performs no reachability check, so a workstation directly behind a wall can still be claimed. While optimized, the villager never walks, pathfinds, or moves toward its workstation on its own, does not actively seek or pathfind to pick up items (items pushed into it by external forces such as flowing water can still be gathered), and no longer opens doors, faces players, wanders, socializes, panics, sleeps, breeds, spreads gossip, or takes part in iron golem summoning; restocking keeps the vanilla work behavior with all vanilla limits preserved, including the daily restock count, the interval between restocks, and the cross-day reset, and existing gossip and reputation keep affecting trade prices. The profession lifecycle keeps vanilla semantics: after the workstation is removed or invalidated, villagers that have never traded and gained no experience lose their profession under the vanilla conditions, while villagers that have traded or gained experience keep their locked profession and lose the job site binding; both automatically claim a matching workstation that appears within roughly 1.73 blocks—an unemployed villager claims any workstation type and gains the matching profession, a locked villager only claims a workstation of its own profession, and nitwits never claim—after which restocking resumes automatically; the whole lifecycle produces no movement or pathfinding, and workstations placed farther away are not claimed. The villager can still be moved by external force, boats, minecarts, or flowing water (it never pathfinds onto vehicles on its own) and still tries to jump out of water; if the villager is pushed out of range before a claim finishes, the unfinished claim is released automatically and retried once the villager is back in range. Name matching is case sensitive and must equal the lowercase name `trade` exactly. After renaming, removing the name, or turning the rule off, the villager recovers its full vanilla AI through the vanilla brain rebuild path, and no mod-dependent data is written to the save.
