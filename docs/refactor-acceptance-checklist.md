...
| itemFrameInvisible | false | F | 全部 | ItemFrameEntityMixin×5（Yarn）/ ItemFrameMixin（mc26x）+ ItemFrameInteractionHelper | 否 |
| itemFrameFixed | false | F | 全部 | 与 itemFrameInvisible 共用同一 mixin / helper | 否 |
| easyWaterloggedBlockPlacement | false | F | 全部 | BlockItemEasyWaterloggedBlockPlacementMixin×2 + platform-mc12111 专用变体 | 否 |
| portableInfiniteWater | false | F,S | 全部 | ItemUsagePortableInfiniteWaterMixin（Yarn）/ ItemUtilsPortableInfiniteWaterMixin（mc26x） | 否 |
...
| neutralPhantoms | false | F,S | 全部 | Yarn：PhantomEntity 版×6 + LivingEntity 版×3 + FindTargetGoal 版×1；Mojmap：Phantom / PhantomAttackPlayerTargetGoal / LivingEntity 三类 | 否 |
...
| machineStatusRollbackWarning | false | — | 全部 | ServerPlayNetworkHandler…Mixin + ServerCommonNetworkHandlerAccessor（mc121x）/ ServerGamePacketListenerImpl…Mixin（26.x）+ Handler 类 + 全局配置 json | 否 |
...
| namedWanderingTraderPersistence | false | F | 全部 | WanderingTraderEntityMixin（Yarn）/ WanderingTraderMixin（mc26x） | 否 |
| villagerTradingOptimization | false | F,O | 全部 | Yarn：VillagerEntity 版×4 + Sensor / EntitySet 版；Mojmap：4 类；VillagerTradingOptimizationTasks 等 5 份 + RuleHelper + common Access | 否 |
| ironGolemSpawningOptimization | false | F,O | 全部 | Yarn 合计 21 份分叉；Mojmap 13 类（mc26x）；common Optimizer / SkipMarked / Access + Hooks | 否 |
| nameTagDuplicateNamingFix | false | B | 全部 | NameTagItemDuplicateNamingFixMixin×3 | 否 |
| mobsSpawnWithoutSpears | false | F | mc12111 / mc261 / mc262 | Piglin + Zombie MobsSpawnWithoutSpearsMixin（Yarn / Mojmap 各 2） | 否 |
...
| craftableCoralBlocks | false | F,S | 全部 | CraftingScreenHandler / CraftingMenu 刷新 Mixin×3 + DataPackController / Validator / ConflictDetector 等 + 10 配方资源包 | 否 |
| waterFluidTickDelay | "5"（{freeze,5}，strict=false） | F | 全部 | WaterFluidTickRateMixin（Yarn）/ WaterFluidTickDelayMixin（mc26x）+ FlowableFluid / FlowingFluidFreezeMixin + FluidSettings / Validator / Util | 否 |
| lavaFluidTickDelay | "30"（{freeze,30}，strict=false） | F | 全部 | LavaFluidTickRateMixin×2（两档内容分叉）/ LavaFluidTickDelayMixin（mc26x）+ 共用 Freeze mixin | 否 |
...
- 注册：`VillagerEventsLogger121`（全部 1.21.x 平台）/ `VillagerEventsLogger26`（mc261 / mc262），入口类 `registerLoggers()` 接入。
