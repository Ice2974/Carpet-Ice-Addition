...
>
> **验收状态：完成**（2026-09-06；P9-0 基线 + C1 verifier no-op 证明 + 双 owner 等强证明 + clean 11 平台构建与全部验证器 + verifyJarEquivalence 11/11 内容级等价，见 §4 / §5；GitHub Actions Build #59 通过（HEAD `77b7117`，见 §5）；Level 3 游戏内人工测试通过（2026-09-06 人工确认，见 §6））。
...
| commit | 内容 |
|---|---|
| `f4348ba` | P9-C1：`verifyJarEquivalence` 两层 ownership 演进（Layer 1 source owners / Layer 2 class entries / major invariant / scoped channel-B / sources 解释收口）+ `selfTestRenameEquivalence` 新增 5 例 ownership-transfer 用例（①–⑤）；未迁移树 no-op 证明（11/11 PASS，ownership 集合为空）。 |
| `3f76129` | P9-C2：原子切换（27 source owner 迁移 + 3 测试迁移 + 测试接线 + 删除 `:common` Java 体系 + 资源路径 + activeJavaRoots / projectsEvaluated / build.yml / 根 `loader_version` 清理）+ transferred-entry scoped channel-B 规范化扩展（2026-09-06 人工授权，见 §2）+ `selfTestRenameEquivalence` 新增 2 例（⑥–⑦，扩至 7 例）。 |
| `77b7117` | C3：本文档 + target-architecture §6/§8 + acceptance-checklist + AGENTS.md 最小现势化。 |
...
- **selfTest 最终 7 例**（C1 新增用例 ①–⑤；C2 经人工授权 lambda 序号规范化后新增 ⑥–⑦）：① transfer + nested `$` + 仅合法 version 差异 PASS（byte-identical / channel-B 双路径）；② 非 ownership class 仅 version 差异 FAIL；③ transferred current major 与 java_release 不符 FAIL；④ literal/instruction 差异 FAIL；⑤ 非 ownership nested `$` class version 差异 FAIL；⑥ transferred lambda 序号差异 PASS；⑦ 同一 lambda 序号差异在非 ownership class 上 FAIL。
...
| CI 状态 | **GitHub Actions Build #59 通过**（workflow `Build`，job `Build and verify all platforms`，HEAD `77b7117`，result success，2026-09-06 人工确认）：`Build and verify` step（`gradlew build :1.21.11:test verifyCraftableCoralBlocksJars verifyFabricModJson verifyMixinConfigs verifyClassRenameMapping selfTestRenameEquivalence --stacktrace`）PASS。`verifyJarEquivalence -PbaselineDir=...` 为本地专用门禁（workflow 不包含该任务），CI 记录仅覆盖远端实际执行并成功的门禁 |
